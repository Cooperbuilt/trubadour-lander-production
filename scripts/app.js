...
/* Click and change animation for poetry feed section. Steps ->
1) Add 'click' animation to social--like
2) Add 'like' animation to social--like
3) Change poem title
4) Change poem body
5) Change poem tags
*/

const like = document.querySelector('.social--like');

const next = document.querySelector('.social--next');

const title = document.querySelector('.section__feed__title');

const body = document.querySelector('.section__feed__body');

const genes = document.querySelector('.genes--tags');

const poemTitleOne = '<strong>Afterword: balls</strong>';

const poemBodyOne =
  "i, too, played croquet</br> with the hedgehog balls.</br></br>and when my flamingo</br>club twisted it's pink nect to</br>look up at me, i was naked.</br></br>so much shame at my shame,</br>that shameless pink bird, never</br> nude & formed to whatever</br>in my grasping pink hands</br>until i laughed and laughed and</br>and couldn't even swing";

const genesOne =
  'Fantasy / Imagination, Dreamlike, Non-erotic, Word / Sound Repetition, Closed Authoring';

const poemTitleTwo = '<strong>Freeway</strong>';

const poemBodyTwo = `
At night the driver in front</br>
throws a cigarette out the window</br>
<span style="margin-left: 40px"></span>and briefly, while it's still</br>
</br>
<span style="margin-left: 40px"></span>glowing on the wind,</br>
<span style="margin-left: 40px"></span>I want to swerve,</br>
</br>
how the cherry looks like the shining eye</br>
of an animal that didn't hear</br>
<span style="margin-left: 40px"></span>us tearing down the road</br>
</br>
<span style="margin-left: 40px"></span>I'm driving because</br>
<span style="margin-left: 40px"></span>I'm always driving</br>
`;

const genesTwo =
  'Short (< 35 lines), Indented structure, Word repetition, Themes of death, Family dischord, Dreamlike';

const poemTitleThree = '<strong>Freeway</strong>';

const poemBodyThree =
  'I love your lips when theyre wet with wine And red with a wild desire;  love your eyes when the lovelight lies Lit with a passionate fire. I love your arms when the warm white flesh Touches mine in a fond embrace; I love <br />your hair when the strands enmesh Your kisses against my face. Not for me the cold, calm kissOf a virgin’s bloodless love; Not for me the saint’s white bliss, Nor the heart of a spotless dove. But give me the love that so freely givesAnd laughs at the whole world’s blame, With your body so young and warm in my arms, It sets my poor heart aflame.';

const genesThree = ' third tag, third tag, third tag, third tag, third tag, third tag,';

poemAnimation = function() {
  // FIRST POEM CHANGE
  // Add like
  setTimeout(function() {
    like.classList.add('social--animation');
  }, 700);
  // Click next button
  setTimeout(function() {
    next.classList.add('social--animation');
  }, 1700);
  // Change poem and remove animations
  setTimeout(function() {
    title.innerHTML = poemTitleOne;
    body.innerHTML = poemBodyOne;
    genes.innerHTML = genesOne;
    like.classList.remove('social--animation');
    next.classList.remove('social--animation');
  }, 2400);
  // SECOND POEM CHANGE
  setTimeout(function() {
    like.classList.add('social--animation');
  ...
  setTimeout(function() {
    title.innerHTML = poemTitleTwo;
    body.innerHTML = poemBodyTwo;
    genes.innerHTML = genesTwo;
    like.classList.remove('social--animation');
    next.classList.remove('social--animation');
  }, 5700);
};

/* Poetry Tag Animation Section. Steps ->
1) Hide the relevant text
2) fade in tags
*/

const fadeOutElements = document.querySelectorAll('.animation--fade');

const fadeInElementOne = document.querySelectorAll('.animation--fadein');

const fadeInElementTwo = document.querySelectorAll('.animation--fadein__tag');

tagAnimation = function() {
  fadeOutElements.forEach(element => {
    element.classList.add('fade');
  });
  setTimeout(function() {
    fadeInElementOne.forEach(element => {
      element.classList.add('fadein');
    });
    fadeInElementTwo.forEach(element => {
      element.classList.add('fadein');
    });
  }, 1000);
};
...
const topOfLearnSection = document.querySelector('.section__feed--white').offsetTop + 500;

const topOfFeedSection = document.querySelector('.yes').offsetTop + 150;
...
function slideAnimation(e) {
  const slidePoint = window.scrollY + window.innerHeight;
  if (slidePoint > topOfLearnSection) {
    tagAnimation();
  }
  if (slidePoint > topOfFeedSection) {
    once(poemAnimation);
  }
}

window.addEventListener('scroll', debounce(slideAnimation));
